...
void CAGrunt :: PrescheduleThink ( void )
{
	if ( ShouldSpeak() )
	{
		if ( m_flNextWordTime < gpGlobals->time )
		{
			...
			do
			{
				num = RANDOM_LONG(0,HL_ARRAYSIZE(pIdleSounds)-1);
...
void CAGrunt :: DeathSound ( void )
{
	...
	EMIT_SOUND ( ENT(pev), CHAN_VOICE, pDieSounds[RANDOM_LONG(0,HL_ARRAYSIZE(pDieSounds)-1)], 1.0, ATTN_NORM );
}
...
void CAGrunt :: AlertSound ( void )
{
	...
	EMIT_SOUND ( ENT(pev), CHAN_VOICE, pAlertSounds[RANDOM_LONG(0,HL_ARRAYSIZE(pAlertSounds)-1)], 1.0, ATTN_NORM );
}
...
void CAGrunt :: AttackSound ( void )
{
	...
	EMIT_SOUND ( ENT(pev), CHAN_VOICE, pAttackSounds[RANDOM_LONG(0,HL_ARRAYSIZE(pAttackSounds)-1)], 1.0, ATTN_NORM );
}
...
void CAGrunt :: PainSound ( void )
{
	...
	EMIT_SOUND ( ENT(pev), CHAN_VOICE, pPainSounds[RANDOM_LONG(0,HL_ARRAYSIZE(pPainSounds)-1)], 1.0, ATTN_NORM );
}
...
void CAGrunt :: Precache()
{
	...
	for ( i = 0; i < HL_ARRAYSIZE( pDieSounds ); i++ )
...
Schedule_t	slAGruntCombatFail[] =
{
	{
		tlAGruntCombatFail,
		HL_ARRAYSIZE ( tlAGruntCombatFail ),
		bits_COND_CAN_RANGE_ATTACK1	|
		bits_COND_CAN_MELEE_ATTACK1,
		0,
		"AGrunt Combat Fail"
...
{
	{
		...
		HL_ARRAYSIZE ( tlAGruntSuppressHornet ),
		0,
		0,
...
Schedule_t	slAGruntRangeAttack1[] =
{
	{ 
		...
		HL_ARRAYSIZE ( tlAGruntRangeAttack1 ), 
		bits_COND_NEW_ENEMY			|
...
Schedule_t	slAGruntHiddenRangeAttack[] =
{
	{ 
		...
		HL_ARRAYSIZE ( tlAGruntHiddenRangeAttack1 ), 
		bits_COND_NEW_ENEMY			|
		bits_COND_HEAVY_DAMAGE		|
		bits_COND_HEAR_SOUND,
...
Schedule_t	slAGruntTakeCoverFromEnemy[] =
{
	{ 
		...
		HL_ARRAYSIZE ( tlAGruntTakeCoverFromEnemy ), 
		bits_COND_NEW_ENEMY,
		0,
...
Schedule_t	slAGruntVictoryDance[] =
{
	{ 
		...
		HL_ARRAYSIZE ( tlAGruntVictoryDance ), 
		bits_COND_NEW_ENEMY		|
...
Schedule_t	slAGruntThreatDisplay[] =
{
	{ 
		...
		HL_ARRAYSIZE ( tlAGruntThreatDisplay ), 
		bits_COND_NEW_ENEMY			|
		bits_COND_LIGHT_DAMAGE		|
		bits_COND_HEAVY_DAMAGE,
