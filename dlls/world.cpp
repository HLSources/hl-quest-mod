...
int CGlobalState::Save( CSave &save )
{
	...
	if ( !save.WriteFields( "GLOBAL", this, m_SaveData, HL_ARRAYSIZE(m_SaveData) ) )
		return 0;
	
	pEntity = m_pList;
	for ( i = 0; i < m_listCount && pEntity; i++ )
	{
		if ( !save.WriteFields( "GENT", pEntity, gGlobalEntitySaveData, HL_ARRAYSIZE(gGlobalEntitySaveData) ) )
			return 0;
...
//
	...
	for ( int i = 0; i < HL_ARRAYSIZE(gDecals); i++ )
